...
class ReportController extends Controller {
    ...
    public function auditSummary(Request $request) {
        ...
        return response()->json([
                    ...
                    'findings' => [
                        ...
                        'by_priority' => DB::table('audit_findings')
    ...
    public function slaCompliance(Request $request) {
        ...
        $defs = SlaDefinition::with(['client', 'department'])->where('status', 'active')->get();
    ...
    public function vendorPerformance(Request $request) {
        ...
        return response()->json([
                    'vendors' => $vendors->map(fn($v) => [
                        ...
                        'avg_eval_score' => $v->evaluations->count() > 0 ? round($v->evaluations->avg('overall_score') ?? $v->evaluations->avg('total_score') ?? null, 1) : null,
                        ...
                        'expiring_contracts' => $v->contracts->where('status', 'active')
                                ->filter(fn($c) => $c->end_date && \Carbon\Carbon::parse($c->end_date)->diffInDays(now()) <= 60 && \Carbon\Carbon::parse($c->end_date)->isFuture())->count(),
                            ])->values(),
