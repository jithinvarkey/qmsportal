...
class AuditController extends Controller {
    ...
    public function show($id) {
        return response()->json(
                        Audit::with(['leadAuditor', 'department', 'program', 'team', 'checklists', 'findings.assignee', 'findings.department','auditTeam'])
                                ->withCount(['findings as open_findings_count' => fn($q) => $q->where('status', 'open')])
                                ->findOrFail($id)
        );
    }
...
//
//    /**
//     * Add a new checklist item to an audit.
//     * Route: POST /api/audits/{id}/checklist
//     */
//    public function storeChecklist(Request $request, string $id): JsonResponse {
//        $request->validate([
...
//        ]);
//
//        $audit = \App\Models\Audit::findOrFail((int) $id);
//
