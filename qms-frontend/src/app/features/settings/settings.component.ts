...
export class SettingsComponent implements OnInit {
  ...
  loadAllCats() {
    ...
    types.forEach(t => {
      this.adm.categories(t).subscribe({
        next: r => { this.allCats[t] = Array.isArray(r)?r:(r?.data??[]); finish(); },
        error: ()  => { this.allCats[t] = [];   finish(); }
      });
    });
  }
  switchCatType(t: CatType) { this.activeCatType.set(t); if (!this.allCats[t]) { this.loadingCats.set(true); this.adm.categories(t).subscribe(r=>{ this.allCats[t]=Array.isArray(r)?r:(r?.data??[]); this.loadingCats.set(false); }); } }
  ...
  submitCat() {
    ...
    req.subscribe({
      next: () => { this.saving.set(false); this.showCatForm.set(false); this.adm.categories(this.catFormType).subscribe(r=>this.allCats[this.catFormType]=Array.isArray(r)?r:(r?.data??[])); },
      error: e => { this.saving.set(false); this.formError.set(e?.error?.message||'Failed.'); }
    });
  }
  deleteCategory(c: any) {
    ...
    this.adm.deleteCategory(this.activeCatType(), c.id).subscribe({
      next: () => {
        ...
        this.adm.categories(this.activeCatType()).subscribe(r => this.allCats[this.activeCatType()] = Array.isArray(r)?r:(r?.data??[]));
      },
