...
@Component({
  ...
  template: `
    <div class="survey-page"
      ...
      <div class="survey-wrapper">
        ...
        <!-- Survey Form -->
        <div class="survey-card" *ngIf="survey() && !submitted() && !loading() && !errorMessage()" [style.background]="branding().card_color">

          <!-- Header -->
          <div class="survey-header" [style.background]="branding().card_color">
            <div class="survey-logo-wrap" >
              <img *ngIf="branding().logo_url" [src]="branding().logo_url" class="survey-logo-img" alt="Logo" width='100'>
              <div *ngIf="!branding().logo_url" class="survey-logo" [style.color]="branding().primary_color">◈ Diamond QMS</div>
            </div>
                ...
                <div class="nps-buttons">
                  <button
                    *ngFor="let n of npsScale()"
                    class="nps-btn"
                    [class.active]="(answers[q.id]?.score ?? -1) === n"
                  [style.background]="(answers[q.id]?.score ?? -1) === n ? branding().primary_color : ''"
  ...
  styles: [`
    ...
    /* ─────────────────────────────────────────────
   Responsive Survey UI Enhancements
───────────────────────────────────────────── */
...
export class SurveyPublicComponent implements OnInit, OnDestroy {
  ...
  // Strip trailing slash so URLs never get double-slash
  private readonly apiBase = (environment.apiUrl || 'http://192.168.100.189:8084/api').replace(/\/+$/, '');
  ...
  recipientName = '';
  comments      = '';
  answers: Partial<Record<number, { answer: any; score?: number }>> = {};
  ...
  ngOnInit(): void {
    ...
    this.svc.publicGetSurvey(token).subscribe({
      next: (res: any) => {
        // Flexible unwrap: handles { data: { survey, branding } } and flat shapes
        const d        = res?.data ?? res;
        const survey   = d?.survey   ?? d;
        const branding = survey?.branding ?? null;
        this.survey.set(survey);

        // Apply branding — use || not ?? so empty strings also fall back to defaults
        this.branding.set({
          logo_url:          branding?.logo_url          ?? null,
          background_color:  branding?.background_color  || '#0f172a',
          background_image:  branding?.background_image  ?? null,
